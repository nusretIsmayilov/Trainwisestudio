...
const getCustomerAccessState = (
...
): { accessLevel: 'A' | 'B' | 'C'; allowedRoutes: string[]; deniedRoutes: string[] } => {
  // Case B: Trial OR subscription - all routes allowed
  if (hasTrial || hasSubscription) {
    return {
      accessLevel: 'B',
      allowedRoutes: ['home', 'programs', 'progress', 'library', 'messages', 'my-coach', 'blog', 'settings'],
      deniedRoutes: [],
    };
  }
  ...
  if (hasCoachContract) {
    return {
      accessLevel: 'C',
      allowedRoutes: ['home', 'programs', 'library', 'messages', 'my-coach', 'settings'],
  ...
  return {
    accessLevel: 'A',
    allowedRoutes: ['home', 'messages', 'my-coach', 'settings'],
    deniedRoutes: ['programs', 'library', 'progress', 'blog'],
  };
};

const AccessLevelContext = createContext<AccessLevelState | undefined>(undefined);
