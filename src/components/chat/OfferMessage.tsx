...
export const OfferMessage: React.FC<OfferMessageProps> = ({
  ...
  const { acceptOffer, rejectOffer, rejectOfferByMessage, loading } =
    useOfferActions();

  const offer = Array.isArray(message.coach_offer)
  ...
  const getStatusBadge = () => {
    ...
    const status = (
      isExpired ? "expired" : offer.status || "pending"
  ...
  const effectiveStatus: "pending" | "accepted" | "rejected" | "expired" =
    isExpired ? "expired" : (offer?.status as any) || "pending";
  const canRespond =
    !isOwn &&
