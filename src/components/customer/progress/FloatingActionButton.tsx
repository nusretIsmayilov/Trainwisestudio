...
const WeightScroller = ({ onWeightChange, value }) => {
  ...
  const handleScroll = (type: "whole" | "decimal") => {
    ...
    if (type === "whole") {
      ...
      console.log("Weight scroller - whole number changed:", {
        newWeight,
        currentDecimal,
        newValue,
      });
      ...
      console.log("Weight scroller - decimal changed:", {
        newDecimal,
        currentWhole,
        newValue,
      });
...
export default function FloatingActionButton() {
  ...
  const actionItems = [
    {
      label: t("progress.weightIn"),
      icon: <Weight className="h-5 w-5" />,
      action: () => null,
    },
    {
      label: t('progress.photo'),
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col-reverse items-end">
      ...
      <Dialog open={isPhotoModalOpen} onOpenChange={setIsPhotoModalOpen}>
        <DialogContent className="max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t('progress.addPhoto')}</DialogTitle>
            <DialogDescription>{t('progress.photoHint')}</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="photo-notes">{t('progress.notesOptional')}</Label>
              <Textarea
                id="photo-notes"
                placeholder= {t('progress.photoNotes')}
                value={photoNotes}
            ...
            >
              ...
              🖼️ {t('progress.selectFromLibrary')}
            </Button>
            <input
              type="file"
              accept="image/*"
      ...
      <Dialog open={isWeightModalOpen} onOpenChange={setIsWeightModalOpen}>
        <DialogContent className="max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t('progress.recordWeight')}</DialogTitle>
            <DialogDescription>
              {getLatestWeight() ? (
                <>
                  {t('progress.previousWeight')}{" "}
                  <span className="font-bold">{getLatestWeight()}kg</span>.
                </>
              ) : (
          ...
          <div className="flex flex-col items-center py-4 space-y-4">
            ...
            <div className="w-full space-y-2">
              <Label htmlFor="weight-notes">{t('progress.notesOptional')}</Label>
              <Textarea
                id="weight-notes"
            ...
            <div className="w-full space-y-2">
              <Label htmlFor="weight-input">{t('progress.manualWeight')}</Label>
              <Input
                id="weight-input"
                type="number"
                step="0.1"
                ...
                value={currentWeight}
                onChange={(e) =>
                  setCurrentWeight(parseFloat(e.target.value) || 70)
                }
                placeholder="70.0"
              />
            </div>
