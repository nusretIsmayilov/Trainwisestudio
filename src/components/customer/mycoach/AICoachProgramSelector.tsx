...
const AICoachProgramSelector: React.FC<AICoachProgramSelectorProps> = ({
  onProgramGenerated,
}) => {
  const { t } = useTranslation();
  const programTypes: ProgramTypeOption[] = useMemo(
  () => [
    {
      id: 'fitness',
      label: t('fitness'),
      description: t('program.fitnessDesc'),
      icon: Dumbbell,
      accent: 'text-[#F97316]',
      accentBg: 'bg-[#F97316]/15',
      gradient:
        'from-white via-white to-[#F97316]/10 dark:from-slate-950 dark:via-slate-950 dark:to-[#F97316]/20',
    },
    {
      id: 'nutrition',
      label: t('nutrition'),
      description: t('program.nutritionDesc'),
      icon: Apple,
      accent: 'text-[#10B981]',
      accentBg: 'bg-[#10B981]/15',
      gradient:
        'from-white via-white to-[#10B981]/10 dark:from-slate-950 dark:via-slate-950 dark:to-[#10B981]/20',
    },
    {
      id: 'mental_health',
      label: t('mental_health'),
      description: t('program.mentalDesc'),
      icon: Brain,
      accent: 'text-[#6366F1]',
      accentBg: 'bg-[#6366F1]/15',
      gradient:
        'from-white via-white to-[#6366F1]/10 dark:from-slate-950 dark:via-slate-950 dark:to-[#6366F1]/20',
    },
  ],
  [t]
);
  ...
  const handleGenerateProgram = async () => {
    ...
    if (!hasPlanAccess) {
      toast.error(
        "AI Coach requires an active subscription or trial. Please upgrade to access this feature.",
      );
      return;
    }
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke(
        "ai-generate-plan",
        {
          body: {
            userId: user.id,
            category:
              selectedType === "mental_health" ? "mental_health" : selectedType,
          },
        },
      );
      ...
      if (
        ...
        errorText.includes("active subscription")
      ) {
        toast.error(
          "AI Coach requires an active subscription or trial. Please upgrade to access this feature.",
        );
        ...
        toast.error(
          errorMessage || "Failed to generate program. Please try again.",
        );
      }
    } finally {
      ...
      <CardContent className="space-y-6">
              ...
              >
                ...
                >
                  <CardContent className="p-6 text-center space-y-4">
                    <div
                      className={cn(
                        "mx-auto w-16 h-16 rounded-full flex items-center justify-center shadow-inner",
                        type.accentBg,
                      )}
                    >
        ...
        <Button
          ...
          className={cn(
            ...
            (!hasPlanAccess || !selectedType || isGenerating) &&
              "pointer-events-none",
          )}
          size="lg"
        >
