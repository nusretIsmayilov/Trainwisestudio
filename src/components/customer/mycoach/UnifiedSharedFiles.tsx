...
const FileList: React.FC<FileListProps & { files: SharedFile[] }> = ({
  isDrawer = false,
  files,
}) => {
  const { t } = useTranslation();
    ...
    return (
      <Card className="shadow-lg border-0 rounded-2xl">
        <CardContent className="flex flex-col items-center justify-center p-12 text-center">
          ...
          <h4 className="font-semibold mb-2">{t("coach.noFiles")}</h4>
...
const UnifiedSharedFiles: React.FC<UnifiedSharedFilesProps> = ({
  className,
  onViewAll,
}) => {
  ...
  if (loading) {
    return (
      <Card
        className={cn(
          "shadow-xl border-0 rounded-2xl animate-fade-in",
          className,
        )}
      >
  ...
  return (
    <div className="h-full overflow-y-auto p-6 space-y-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="text-xl font-bold">{t("coach.sharedFiles")}</h3>
          <p className="text-sm text-muted-foreground">
            {sharedFiles.length} file{sharedFiles.length !== 1 ? "s" : ""} from
            your coach
          </p>
        </div>
      </div>
