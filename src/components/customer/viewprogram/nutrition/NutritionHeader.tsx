// src/components/customer/viewprogram/nutrition/NutritionHeader.tsx
...
import { Apple, ListChecks, Beef } from "lucide-react";
...
export default function NutritionHeader({ task }: { task: DetailedNutritionTask }) {
    ...
    <div className="relative w-full h-80 overflow-hidden rounded-3xl shadow-xl">
      ...
      <div className="relative flex h-full flex-col justify-end p-8 text-white">
        ...
        <div className="mt-6 grid grid-cols-3 gap-4 border-t border-white/20 pt-6">
          <Stat icon={Apple} label="Meals" value={task.meals.length} />
          <Stat icon={ListChecks} label="Calories" value={task.totalCalories} />
          <Stat icon={Beef} label="Protein" value={`${task.totalProtein}g`} />
        </div>
      </div>
    </div>
  );
