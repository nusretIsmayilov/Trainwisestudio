...
const ClientList = ({ refreshTrigger }: ClientListProps) => {
  ...
  const applyFilters = (
    clients: any[],
    filters: { status: string[]; badges: string[] },
  ) => {
    ...
    if (filters.badges.length > 0) {
      filtered = filtered.filter((client) => {
        return filters.badges.some(
          (badge) => client.badges[badge as keyof typeof client.badges],
        );
      });
    }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        {filteredClients.length === 0 && (
          <div className="col-span-full text-center py-12">
            <div className="text-sm text-muted-foreground">
              {allClients.length === 0
                ? t("clients.empty.noClientsYet")
                : t('clients.empty.noClientsMatchFilters')}
            </div>
          </div>
        )}
      </div>
