...
const NutritionDay: React.FC<NutritionDayProps> = ({
  day,
  data,
  onDataChange,
  onAddClick,
  ...
  const handleRemoveItem = useCallback(
    (section: MealSection, itemIndex: number) => {
  ...
  const handleMove = useCallback(
    (section: MealSection, index: number, direction: "up" | "down") => {
      const newItems = [...data[section]];
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4 mt-3">
        ...
        <div className="space-y-1">
          <Label htmlFor={`time-${item.id}`} className="text-xs">
            Time (Optional)
          </Label>
          <div className="relative">
            ...
            <Input
              ...
              value={item.time || ""}
              onChange={(e) =>
                handleUpdateItem(section, itemIndex, "time", e.target.value)
              }
        ...
        {mealSections.map((section) => (
          <div key={section} className="space-y-3">
              ...
              <AnimatePresence>
                {data[section].length > 0 ? (
                  data[section].map((item, itemIndex) =>
                    renderRecipeCard(item, itemIndex, section),
                  )
                ) : (
                  <div className="text-center text-muted-foreground py-4">
                    <Utensils className="h-8 w-8 mx-auto mb-2 text-muted-foreground/30" />
                    <p className="font-semibold capitalize">
                      Add recipes to your {section.replace("snack", " Snack")}{" "}
                      plan.
                    </p>
                  </div>
                )}
            ...
            <Button
              ...
              onClick={onAddClick}
            >
              <Plus className="h-4 w-4" /> Add Recipe to{" "}
              {section.replace("snack", " Snack")}
            </Button>
          </div>
        ))}
