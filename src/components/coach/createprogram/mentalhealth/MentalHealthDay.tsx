...
interface MentalHealthDayProps {
  ...
  onDataChange: (data: {
    [key in MentalHealthSection]: MentalHealthDayItem[];
  }) => void;
  onAddClick: () => void;
...
const mentalHealthSections: MentalHealthSection[] = [
  "morning",
  "evening",
  "night",
];

const MentalHealthDay: React.FC<MentalHealthDayProps> = ({
  day,
  data,
  onDataChange,
  onAddClick,
  ...
  const handleUpdateItem = useCallback(
    (
      ...
      itemIndex: number,
      field: keyof MentalHealthDayItem,
      value: string,
    ) => {
  ...
  const handleRemoveItem = useCallback(
    (section: MentalHealthSection, itemIndex: number) => {
  ...
  const handleMove = useCallback(
    (section: MentalHealthSection, index: number, direction: "up" | "down") => {
      const newItems = [...data[section]];
  ...
  const renderActivityCard = (
    ...
    >
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <div className="flex flex-col gap-1">
            <Button
              ...
              onClick={() => handleMove(section, itemIndex, "up")}
            ...
            <Button
              ...
              size="icon"
              className="h-4 w-4 p-0 hover:bg-muted"
              onClick={() => handleMove(section, itemIndex, "down")}
              disabled={itemIndex === data[section].length - 1}
            >
      ...
      <div className="flex items-center gap-4 text-sm font-medium text-muted-foreground border-b pb-2 mb-4">
        ...
        <span className="capitalize">Type: {item.activity.type}</span>
      </div>

      {/* Comment / Reflection Input */}
      <div className="mt-4 space-y-2">
        <Label htmlFor={`comment-${item.id}`}>
          Notes / Intention (Optional)
        </Label>
        <Textarea
          id={`comment-${item.id}`}
          ...
          value={item.comment}
          onChange={(e) =>
            handleUpdateItem(section, itemIndex, "comment", e.target.value)
          }
        />
      </div>
    </motion.div>
