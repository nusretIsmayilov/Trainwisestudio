  ...
  return (
    ...
    >
      <Card className="rounded-xl overflow-hidden shadow-2xl transition-all duration-300 hover:scale-[1.02] hover:shadow-primary/20 cursor-pointer flex flex-col h-full">
          ...
          <div className="absolute top-0 right-0 p-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            {onView && (
              <Button
                variant="ghost"
                size="icon"
                className="h-8 w-8 bg-background/70 hover:bg-background/90 text-blue-600"
                onClick={(e) => {
                  e.stopPropagation();
                  onView(item);
                }}
            ...
            <Button
              variant="ghost"
              size="icon"
              className="h-8 w-8 bg-background/70 hover:bg-background/90 text-primary"
              onClick={(e) => {
                e.stopPropagation();
                onEdit(item);
              }}
            ...
            <Button
              variant="ghost"
              size="icon"
              className="h-8 w-8 bg-background/70 hover:bg-background/90 text-destructive"
              onClick={(e) => {
                e.stopPropagation();
                onDelete(item.id);
              }}
        ...
        <CardContent
          className="p-4 space-y-2 flex flex-col justify-between flex-grow"
          style={{ minHeight: "130px" }}
        >
          <div>
            <div className="flex items-start justify-between mb-1">
              {/* Using text-lg and font-bold for CardTitle equivalent */}
              <h3 className="text-xl font-bold leading-tight line-clamp-2 pr-2">
                {item.name}
              </h3>
