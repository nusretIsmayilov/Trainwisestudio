...
const BlogContentBuilder: React.FC<BlogContentBuilderProps> = ({
  ...
  const updateItem = useCallback(
    (id: string, value: string) => {
      onContentChange(
        content.map((item) => (item.id === id ? { ...item, value } : item))
      );
    },
  ...
  const renderContentBlock = (item: BlogContentItem, index: number) => {
      ...
      >
        ...
        <div className="flex flex-col items-center pt-2 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 w-6">
          ...
          <Button
            ...
            size="icon"
            className={cn(
              "h-6 w-6 text-muted-foreground hover:bg-muted mb-1",
              index === 0 && "invisible"
            )}
          ...
          <Button
            ...
            size="icon"
            className={cn(
              "h-6 w-6 text-muted-foreground hover:bg-muted mt-1",
              index === content.length - 1 && "invisible"
            )}
          ...
          <Button
            variant="ghost"
            size="icon"
            className="h-8 w-8 mt-2 text-destructive hover:bg-destructive/10"
