...
const AccessControl = ({
  ...
  const { isLoading, isAuthenticated, canAccessRoute, getAccessDenialReason } =
    useAccessLevel();
  ...
  if (!hasAccess) {
    ...
    const isPaymentRequired =
      denialReason?.includes("subscription") || denialReason?.includes("trial");
    ...
    return (
      <div className="relative isolate min-h-[60vh]">
        ...
        <div className="absolute inset-0 flex items-center justify-center px-4 py-10 bg-background/95 backdrop-blur-xl z-20">
          <div
            className={`text-center p-6 max-w-md mx-auto rounded-2xl border ${
              isPaymentRequired
