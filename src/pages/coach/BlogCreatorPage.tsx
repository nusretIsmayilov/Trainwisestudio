...
import {
  ...
  BlogCategory,
  CATEGORY_DETAILS,
} from "@/mockdata/blog/mockBlog";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { Label } from "@/components/ui/label";
import { cn } from "@/lib/utils";
import BlogContentBuilder, {
  BlogContentItem,
} from "@/components/coach/blog/BlogContentBuilder";
  ...
  const ensureShape = (arr: any[]): BlogContentItem[] => {
    return arr.filter(Boolean).map((it: any, idx: number) => {
      ...
      return {
        ...
        value:
          typeof it?.value === "string" ? it.value : String(it?.value ?? ""),
      } as BlogContentItem;
    });
  };
...
const BlogCreatorPage: React.FC<BlogCreatorPageProps> = ({
  onBack,
  onSubmit,
  initialPost,
}) => {
  ...
  const currentCategory: BlogCategory =
    (formData.category as BlogCategory) || "fitness";
  ...
  const handleFileChange = async (
    ...
    try {
      ...
      if (uploadError) {
        ...
        toast.error(
          "Failed to upload image. Please try again or check if the storage bucket exists.",
        );
        event.target.value = "";
        return;
      }
    ...
    >
      ...
      <div className="flex items-center justify-between pb-6 border-b">
        ...
        >
          ...
          {isEditing ? t("profile.saveChanges") : "Publish Post"}
        </Button>
      </div>
      ...
      <div
        className={cn(
          ...
          !hasUserImage
            ? "border-primary/30 bg-gray-100 dark:bg-gray-800"
            : "border-transparent",
        )}
        ...
        <div className="absolute top-4 right-4 z-10 flex gap-2">
          {hasUserImage && (
            <Button
              variant="destructive"
              size="icon"
              className="rounded-full h-7 w-7 bg-black/50 hover:bg-black/80"
          ...
          <Button
            variant="default"
            size="icon"
            className="rounded-full h-7 w-7 bg-black/50 hover:bg-black/80 text-white"
            onClick={(e) => {
              e.stopPropagation();
              triggerFileInput();
            }}
        ...
        <div className="space-y-6">
          <div className="space-y-3">
            ...
            <ToggleGroup
              ...
              onValueChange={(value) =>
                handleFormChange("category", value as BlogCategory)
              }
                  ...
                  <ToggleGroupItem
                    ...
                    className={cn(
                      ...
                      formData.category === cat
                        ? `${detail.color} text-white hover:${detail.color}/90 border-transparent`
