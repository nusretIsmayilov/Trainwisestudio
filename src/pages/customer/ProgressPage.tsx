...
export default function ProgressPage() {
  ...
  const { personalization, loading: personalizationLoading } =
  ...
  const personalizationInsights =
    personalization?.insights?.map((insight) => ({
      id: insight.id,
      text: insight.text,
      type: insight.type,
      emoji: insight.emoji || "💡",
  ...
  const insightsToRender = hasAIInsights
    ? personalizationInsights
    : data.smartInsights || [];
  const shouldShowInsights =
    hasAIInsights ||
    (hasMinimumData && insightsToRender && insightsToRender.length > 0);
  ...
  const avgEnergy =
    last7DaysCheckins.length > 0
      ? last7DaysCheckins.reduce((sum, day) => sum + (day.energy || 0), 0) /
        last7DaysCheckins.length
      : 0;
  ...
  const avgProtein =
    last7DaysMacros.length > 0
      ? last7DaysMacros.reduce((sum, day) => sum + day.protein, 0) /
        last7DaysMacros.length
      : 0;
  const avgCarbs =
    last7DaysMacros.length > 0
      ? last7DaysMacros.reduce((sum, day) => sum + day.carbs, 0) /
        last7DaysMacros.length
      : 0;
