...
const BlogPage = () => {
  ...
  const uiPosts = (posts || []).map((p: any) => {
          ...
          .map((b) => {
              ...
              const textVal = (() => {
                ...
                if (Array.isArray(pv))
                  return pv
                    ...
                    .join("\n\n");
                if (pv && typeof pv === "object" && "value" in pv)
                  return pv.value;
            ...
            if (b.type === "file") {
              if (b.mediaType === "image")
                return `<img src="${b.value}" alt="" />`;
              if (b.mediaType === "video")
                return `<video src="${b.value}" controls></video>`;
            }
            return "";
          })
          .join("");
    ...
    const readTimeMins = Math.max(
      1,
      p.read_time || Math.ceil(plainTextLength / 500)
    );
                  ...
                  {latestPost && (
                    <FeaturedPost
                      post={{
                        ...
                        excerpt:
                          (latestPost as any).introduction ||
                          latestPost.excerpt,
                        imageUrl:
                          (latestPost as any).cover_url ||
                          latestPost.imageUrl ||
                          "https://placehold.co/1024x512",
                        author: {
                          name:
                            (latestPost as any).author_name ||
                            latestPost.author.name,
                          avatarUrl:
                            (latestPost as any).author_avatar ||
                            latestPost.author.avatarUrl ||
                            "https://placehold.co/64x64",
                          id:
                            (latestPost as any).author_id ||
                            latestPost.author.id,
                        },
                        date: (latestPost as any).updated_at
                          ? new Date(
                              (latestPost as any).updated_at
                            ).toLocaleDateString()
                          : latestPost.date,
                        readTime: (latestPost as any).read_time
                          ? `${(latestPost as any).read_time} min read`
                          : latestPost.readTime,
                  ...
                  <BlogTimeline
                    posts={previousPosts.map((p) => ({
                      ...
                      imageUrl:
                        (p as any).cover_url ||
                        p.imageUrl ||
                        "https://placehold.co/512x256",
                      author: {
                        name: (p as any).author_name || p.author.name,
                        avatarUrl:
                          (p as any).author_avatar ||
                          p.author.avatarUrl ||
                          "https://placehold.co/32x32",
                        id: (p as any).author_id || p.author.id,
                      },
                      date: (p as any).updated_at
                        ? new Date((p as any).updated_at).toLocaleDateString()
                        : p.date,
                      readTime: (p as any).read_time
                        ? `${(p as any).read_time} min read`
                        : p.readTime,
                      slug: p.id || p.slug,
                      content: p.content,
                ...
                {latestPost && (
                  <FeaturedPost
                    post={{
                      title: latestPost.title,
                      excerpt:
                        (latestPost as any).introduction || latestPost.excerpt,
                      imageUrl:
                        (latestPost as any).cover_url ||
                        latestPost.imageUrl ||
                        "https://placehold.co/1024x512",
                      author: {
                        ...
                        avatarUrl:
                          ...
                          "https://placehold.co/64x64",
                        id:
                          (latestPost as any).author_id || latestPost.author.id,
                      },
                      date: (latestPost as any).updated_at
                        ? new Date(
                            (latestPost as any).updated_at
